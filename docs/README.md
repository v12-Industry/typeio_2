...
- [`development/haskell-style.md`](development/haskell-style.md) — the Haskell conventions Fourmolu can't decide: chaining with `.`/`$` instead of nesting parentheses (and the shapes where the parentheses stay), flattening several fallible steps into one `EitherT` block with a single `case` at the edge, and what the `handler`/`template*`/`query*`/`validate*`/`formTo*Form` prefixes mean.
- [`development/backend/routing.md`](development/backend/routing.md) — the API as a Servant type: how a route is declared, how parameters are parsed and refused, and how `App.Link` derives URLs from it.
- [`development/backend/environment.md`](development/backend/environment.md) — the `Env` record (config, logger, DB pool) acquired once at startup, and how a handler reaches it through `AppM`.
- [`development/backend/handlers.md`](development/backend/handlers.md) — what a handler is, where it lives, what `App.Handler` shares between them, and where validation happens.
...
- [`architecture/graph-rendering.md`](architecture/graph-rendering.md) — the *layered* layout engine's pipeline and rendering, used by the Rootless visualization: module map, per-phase contracts and invariants, coordinate and edge-direction conventions, the DOM contract, viewport behaviour and testing.
...
- [`solution-proposals/servant-migration.md`](solution-proposals/servant-migration.md) — **decided**: whether to keep the hand-rolled WAI/Warp web layer or move to a framework, weighed against an audit that found the router matches path prefixes, request parsing goes through `Read`, and URLs are concatenated by hand — resolved as Servant, stood up alongside and ported by domain, with Lucid, persistent/esqueleto and all three test suites carried over and all schema changes deferred.
- [`solution-proposals/telemetry.md`](solution-proposals/telemetry.md) — **decided**: what the app has for observability today (structured logging, no metrics, no tracing) and what it's missing, weighed against Grafana and GCP both being on the horizon but undecided, resolved as OpenTelemetry (metrics + traces via OTLP) so the backend choice is deferred to exporter config rather than instrumentation code.
