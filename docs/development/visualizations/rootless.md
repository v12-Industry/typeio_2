...
- **Each node coloured by its status.** The shape carries
  ...
  beside its `.work` class — `class="work status-active"` — and
  `manage-project.css` turns that into a fill from the
  ...
  label sits directly on the fill. A node whose status is outside those
  four — a row `Domain.Project.Node.Status` has no constructor for —
  carries no status class at all and keeps the plain `.work` fill rather
  than vanishing.
